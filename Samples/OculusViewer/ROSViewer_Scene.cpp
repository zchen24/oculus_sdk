...
void ROSViewerApp::InitMainFilePath()
{
#if 0
...
void ROSViewerApp::PopulateScene(const char *fileName)
{    
...
void ROSViewerApp::PopulatePreloadScene()
{
...
//    String fileName = MainFilePath;
    ...
    Ptr<Texture> imageTex;
    if (imageFile->IsValid())
        imageTex = *LoadTextureTga(mRender, imageFile);
    ...
    if (imageTex)
    {
        ...
        Ptr<ShaderFill> fill = *new ShaderFill(*mRender->CreateShaderSet());
        fill->GetShaders()->SetShader(mRender->LoadBuiltinShader(Shader_Vertex, VShader_MVP));
        fill->GetShaders()->SetShader(mRender->LoadBuiltinShader(Shader_Fragment, FShader_Texture));
        fill->SetTexture(0, imageTex);
        m->Fill = fill;
...
void ROSViewerApp::RenderAnimatedBlocks(ovrEyeType eye, double appTime)
{
...
void ROSViewerApp::RenderGrid(ovrEyeType eye)
{
